...
auto hookPresentD3D12(IDXGISwapChain3* ppSwapChain, UINT syncInterval, UINT flags) -> HRESULT {
    ...
    RenderUtils::deviceType = SwapChain_DeviceType::INVALID_DEVICE_TYPE;
    ...
    if(SUCCEEDED(ppSwapChain->GetDevice(IID_PPV_ARGS(&d3d11Device)))) {
        RenderUtils::deviceType = SwapChain_DeviceType::D3D11;
    } else if(SUCCEEDED(ppSwapChain->GetDevice(IID_PPV_ARGS(&d3d12Device)))) {
        RenderUtils::deviceType = SwapChain_DeviceType::D3D12;
    };

    if(RenderUtils::deviceType == SwapChain_DeviceType::INVALID_DEVICE_TYPE) {
        Utils::debugLog("Failed to get device!");
        goto out;
    };
    
    if(RenderUtils::deviceType == SwapChain_DeviceType::D3D11) {
    ...
    } else if(RenderUtils::deviceType == SwapChain_DeviceType::D3D12) {
